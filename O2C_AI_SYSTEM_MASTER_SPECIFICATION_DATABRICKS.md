# O2C Delivery Risk Copilot: Databricks Master Architecture Specification & Runtime Guide
...
- [SECTION I: SYSTEM ARCHITECTURE & UNIFIED 3-TIER TOPOLOGY](#section-i-system-architecture--unified-3-tier-topology)
  ...
  - [1.2 Unified 3-Tier End-to-End System Topology (Databricks Native View)](#12-unified-3-tier-end-to-end-system-topology)
...
> [!NOTE]
> **Databricks Optimized Edition:** This document has been specially configured for Databricks Workspaces, Databricks Repos, and Databricks Notebook `%md` markdown cells. All architectural topologies, decision flowcharts, and sequence traces use **Databricks Native Monospace Unicode Architecture Blocks** (`text`) that render crisply and reliably across all cloud cluster environments without external JavaScript dependencies, CDNs, or rendering failures.
...
```text
┌────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                        TIER 1: REAL-TIME SENSORY INGESTION & DOCUMENT SYNTHESIS                        │
└────────────────────────────────────────────────────────────────────────────────────────────────────────┘
  [Open-Meteo & OWM Telemetry] ──► [WeatherService] ──► [DatabaseManager] ──► weather_readings (130+ rows)
  [Google News RSS Feed]       ──► [NewsService]    ──► [DatabaseManager] ──► strike_news (396+ rows)
                                                               │
                                                               ▼
  [WeatherPolicyGenerator] ──► 6 Word Weather Protocols ([RULE-W-*])
  [StrikeIntelGenerator]   ──► 17 Word Strike Briefs ([RULE-S-*])

                                            │
                                            ▼
┌────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                        TIER 2: SAP FEATURE STORE, TWO-STAGE ML & HYBRID RAG                            │
└────────────────────────────────────────────────────────────────────────────────────────────────────────┘
  [10 SAP ERP CSV Exports] ──► [MLDatabaseExtension] ──► 19-Feature Vector Space
                                                                │
         ┌──────────────────────────────────────────────────────┴───────────────────────────────────┐
         ▼                                                                                          ▼
  [Stage 1: RandomForestClassifier Gate]                                              [82 Corpus Policy Documents]
  (100 Trees, Depth 6, Class-Weighted)                                                              │
         │                                                                                          ▼
         ├───────────────────────────────────┐                                      [DocumentLoader & ClauseChunker]
         ▼ (If P(delay) < 0.40)              ▼ (If P(delay) >= 0.40)                                │
  [PREDICTED ON TIME]                 [Stage 2: Conditional Huber Regressor]                        ▼
  • Delay: 0.00h (Ghost Eliminated)   (GradientBoostingRegressor, loss='huber')            [909 Semantic Chunks]
  • Risk: $0.00                              │                                                      │
                                             ▼                                          ┌───────────┴───────────┐
                                      [Estimated Delay: >= 12.0h]                       ▼                       ▼
                                             │                                  [Dense FAISS FlatIP]   [Sparse BM25]
                                             ▼                                  (384-dim Vectors)      (Exact Terms)
                                      [Live Environmental Modifiers]                    │                       │
                                      • Live Heat/Rain: +12h, +0.10 P(delay)            └───────────┬───────────┘
                                      • Live Strikes:   +12h, +0.10 P(delay)                        ▼
                                             │                                            [Hybrid RRF Fusion Search]
                                             └───────────────────────────────┬────────────────────────┘
                                                                             ▼
                                                             [Enriched Multi-Engine Risk Payload]

                                                                             │
                                                                             ▼
┌────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                        TIER 3: MULTI-AGENT REASONING & PHYSICAL EXECUTION                              │
└────────────────────────────────────────────────────────────────────────────────────────────────────────┘
                                                         [AgenticOrchestrator]
                                                                 │
         ┌───────────────────────────────┬───────────────────────┴───────────────────────┬────────────────────────┐
         ▼                               ▼                                               ▼                        ▼
  [RouteSupervisorAgent]    [ClinicNotificationDispatcher]                    [ContractAdjudicatorAgent]   [QualityMitigationAgent]
  • GPS Telematics Audit    • Proactive 12h Early Warning                     • Platinum: $500/day SLA     • $1,000 Air Freight Pallet
  • $200 Penalty if Dead    • Secures Statutory Force Majeure                 • Post-17:00: $150 Dock Fee  • QA Hold LIFSK='01'
                                         │                                    • 72h Force Majeure Waiver   • $500 Director Approval Gate
                                         └───────────────────────┬───────────────────────┘                        │
                                                                 ▼                                                ▼
                                                   [LLMReasoningEngine Synthesis] ◄───────────────────────────────┘
                                                   (Qwen2.5 / Databricks / Local)
                                                                 │
                                         ┌───────────────────────┴───────────────────────┐
                                         ▼                                               ▼
                         [SAPActionExecutor Write-Backs]                    [MSTeamsDispatcher Cards]
                         • Delivery Block: VBAK-LIFSK='01'                  • Adaptive Cards v1.4
                         • Delivery Date:  VBAK-VDATU                       • Interactive Action Buttons
                         • Carrier AP:     SAP_BKPF Debit Memo              • 2-Hour Response SLA
                                         │                                               │
                                         └───────────────────────┬───────────────────────┘
                                                                 ▼
                                                [Daily Executive Audit Report]
                                                (daily_agent_report_YYYY-MM-DD.json)
```

> **💡 Databricks Interactive Rendering Tip:** To render this topology graphically in a Databricks Notebook Python cell:
> ```python
> displayHTML('''
...
>         A1["Open-Meteo & OWM Live Telemetry"] --> A2["WeatherService"]
>         B1["Google News RSS Disruption Stream"] --> B2["NewsService"]
>         A2 & B2 --> C1["DatabaseManager (SQLite)"]
>         C1 --> D1["weather_readings"] & D2["strike_news"]
>         D1 --> E1["WeatherPolicyGenerator"] --> F1["6 Word Weather Protocols"]
>         D2 --> E2["StrikeIntelligenceGenerator"] --> F2["17 Word Strike Briefs"]
>     end
>     subgraph "TIER 2: PREDICTIVE ML & RAG"
>         G1["10 SAP ERP Tables"] --> G2["MLDatabaseExtension"] --> G3["19-Feature Vector Space"]
>         G3 --> H1["Two-Stage Hurdle ML Engine"] --> H2["P(delay), Delay Hours, XAI Attributions"]
>         F1 & F2 & J1["82 Corpus Documents"] --> J2["DocumentLoader & ClauseChunker"] --> J3["909 Chunks"]
>         J3 --> K1["FAISS Dense Vectors"] & K2["Okapi BM25 Lexicon"] --> K3["Hybrid RRF Search"]
>         H2 & K3 --> L1["Enriched Multi-Engine Risk Payload"]
>     end
>     subgraph "TIER 3: AGENTS & EXECUTION"
>         L1 --> M1["AgenticOrchestrator"]
>         M1 --> N1["RouteSupervisorAgent ($200 Penalty)"]
>         M1 --> N2["ClinicNotificationDispatcher (12h Early Warning)"]
>         M1 --> N3["ContractAdjudicatorAgent (SLA Math & Waivers)"]
>         M1 --> N4["QualityMitigationAgent ($1,000 Air Freight)"]
>         N1 & N2 & N3 & N4 --> P1["LLMReasoningEngine"]
>         P1 --> Q1["SAPActionExecutor (VBAK-LIFSK, VDATU, Debit Memos)"]
>         P1 --> Q2["MSTeamsDispatcher (Adaptive Cards v1.4)"]
>         P1 --> Q3["Daily Executive Audit Report (daily_agent_report.json)"]
>     end
> </div>
> ''')
> ```
...
```text
  [Live Open-Meteo / OWM API]                [Google News RSS Feeds]
              │ (HTTP GET JSON)                         │ (HTTP GET XML)
              ▼                                         ▼
  [modules/weather_service.py]              [modules/news_service.py]
  • fetch_current()                         • fetch()
  • _owm_one() / _meteo_current_one()       • _build_queries() / _classify_severity()
              │                                         │
              └────────────────────┬────────────────────┘
                                   ▼
                      [modules/database_manager.py]
                      • session_start("full_cycle")
                      • write_weather() / write_strikes()
                                   │
                                   ▼
                      [SQLite Storage: monitor.db]
                      ┌────────────────────────────────────────┐
                      │ • weather_readings (130+ records)      │
                      │ • strike_news      (396+ records)      │
                      │ • ingestion_sessions (audit trail)     │
                      └────────────────────────────────────────┘
                                   │
         ┌─────────────────────────┴─────────────────────────┐
         ▼                                                   ▼
  [modules/weather_policy_generator.py]       [modules/strike_intelligence_generator.py]
  • 5 City Protocols (HYD, BOM, MAA, PNQ, BLR) • 9 City Strike Briefs
  • 1 Master National Weather Protocol        • 7 Transport Modality Briefs
  • Emits [RULE-W-*] Regulation Codes         • 1 Master Intelligence Brief
         │                                                   │
         └─────────────────────────┬─────────────────────────┘
                                   ▼
                    [Output: 23 Word .docx Files]
                    (Preserved in RAG Knowledge Corpus)
```
...
```text
                       [Raw SAP Relational Data (62,299 records)]
                                           │
                                           ▼
         ...
         ┌─────────────────────────────────┴─────────────────────────────────┐
         ▼                                                                   ▼
  [Stage 1: Classification Training]                          [Stage 2: Conditional Regression Training]
  • Full Dataset (62,299 rows)                                • Delayed Subset ONLY (is_delayed == 1)
  • Target: is_delayed (0 or 1)                               • Target: delay_hours (>= 12.0h)
  • Model: RandomForestClassifier                             • Model: GradientBoostingRegressor
    - n_estimators=100, max_depth=6                             - loss='huber', learning_rate=0.08
    - class_weight='balanced'                                   - n_estimators=100, max_depth=5
         │                                                                   │
         ▼                                                                   ▼
  [Output: rf_classifier.pkl]                                 [Output: gb_regressor.pkl]
  • Accuracy: 97.10%                                          • Regressor MAE: 5.63 Hours
  • ROC-AUC: 0.9958                                           • Ghost False Alarms: 0.00 Hours
```
...
```text
  ┌────────────────────────────────────────────────────────┐
  │ 1. STATIC SUPERVISED ML (19 Canonical ERP Features)    │
  └────────────────────────────────────────────────────────┘
    Historical SAP ERP Tables (62,299 Orders)
              │
              ▼
    Two-Stage Hurdle ML Models (RandomForest + Huber Regressor)
              │
              ▼
    Baseline Predictions:
    • P(delay) = 0.77
    • Baseline Delay = 43.0 Hours
              │
              ├───────────────────────────────────────────────────────┐
              ▼                                                       ▼
  ┌──────────────────────────────────────────────┐    ┌──────────────────────────────────────────────┐
  │ 2. DYNAMIC LIVE MODIFIERS (Zero-Leakage)     │    │ 3. LEGAL RAG CONTEXT (Engine B)              │
  └──────────────────────────────────────────────┘    └──────────────────────────────────────────────┘
    Live Weather & Strike Telemetry from SQLite         Contract & Policy Retrieval
    • Severe Heatwave (>40°C): +12.0h, +0.10 P(delay)   • Clause 4.2: Force Majeure Relief
    • Transport Strike Alert:  +12.0h, +0.10 P(delay)   • Section 8.4: 72h Penalty Waiver
              │                                                       │
              └───────────────────────────────┬───────────────────────┘
                                              ▼
                               [FINAL ENRICHED PREDICTION]
                               • Calibrated P(delay) = 0.87 (87.1%)
                               • Calibrated Delay = 67.0 Hours
                               • Force Majeure Candidate = YES
                               • Financial SLA Penalty = WAIVED
```
...
```text
  [82 Raw Corpus Policy Documents (DOCX, PDF, XLSX, TXT)]
                           │
                           ▼
  [modules/rag_engine.py: DocumentLoader]
  • Reads table cells, clause headers, and text formatting
                           │
                           ▼
  [modules/rag_engine.py: ClauseAwareChunker]
  • Two-stage hierarchical splitting along legal sections
  • Targets 500 chars with 50-char overlapping window
                           │
                           ▼
  [909 Semantic Chunks (all_chunks.json)]
                           │
         ┌─────────────────┴─────────────────┐
         ▼                                   ▼
  [Dense FAISS FlatIP]                [Sparse Okapi BM25]
  • SentenceTransformer               • Robertson-Spärck Jones BM25
  • 384-dimensional dense vectors     • k1=1.5, b=0.75
  • L2-normalized Inner Product       • Exact keyword & clause ID matching
         │                                   │
         └─────────────────┬─────────────────┘
                           ▼
  [Reciprocal Rank Fusion (RRF Search)]
  • Score = 1/(60 + Rank_FAISS) + 1/(60 + Rank_BM25)
  • Eliminates false negatives from pure vector search
                           │
                           ▼
  [Top-K Authoritative Legal Policy Chunks]
  (Attached to order risk evaluation for multi-agent reasoning)
```
...
```text
Pipeline            MLDatabaseExtension        Engine A (ML)             Engine B (RAG)          LLMSynthesizer
   │                         │                       │                         │                       │
   │── get_order_details() ─►│                       │                         │                       │
   │◄─ 19-Feature Vector ────│                       │                         │                       │
   │                         │                       │                         │                       │
   ...
   │                                                                                                   │
   │── synthesize_decision() ─────────────────────────────────────────────────────────────────────────►│
   │◄─ Consolidated Executive Resolution Artifact ─────────────────────────────────────────────────────│
```
...
```text
  [Prediction Payload: Order ID, Carrier, Tier, ETA, Root Cause, RAG Rules]
                                     │
                                     ▼
                    ...
                    ┌────────────────┴────────────────┐
                    ▼ (Lost > 12h)                    ▼ (Active)
          [Charge $200 Penalty]              [Verify Speed <= 55 km/h]
          [Void Force Majeure]               [Flag Corridor Hazards]
                    │                                 │
                    └────────────────┬────────────────┘
                                     ▼
                      [ClinicNotificationDispatcher]
                      • Sends Proactive 12h Early Warning
                      • Secures Statutory Force Majeure Legal Defense
                                     │
                                     ▼
                         [ContractAdjudicatorAgent]
                                     │
                    ┌────────────────┴────────────────┐
                    ▼ (FM Verified & 12h Notice)      ▼ (Standard Late Delivery)
          [Grant 72h Penalty Waiver]         [Calculate Tiered Penalties]
          [Waive $500/day SLA Penalty]       • Platinum: $500/day
                                             • Gold: 5%/day (Capped 25%)
                                     │
                    ┌────────────────┴────────────────┐
                    ▼ (ETA >= 17:00)                  ▼ (ETA < 17:00)
          [Assess $150 Dock Overtime Fee]    [No Dock Fee Assessed]
                                     │
                                     ▼
                          [QualityMitigationAgent]
                                     │
                    ┌────────────────┴────────────────┐
                    ▼ (Specialty Diet & Delay > 48h)  ▼ (Heatwave > 40°C or Shelf-Life < 6m)
          [Authorize $1,000 Air Freight]     [Mandate QA Quarantine Hold '01']
                                             [Flag for HPLC Potency Lab Assay]
                                     │
                    ┌────────────────┴────────────────┐
                    ▼ (Expense > $500 or Penalty > $1,000) ▼ (Expense <= $500)
          [DIRECTOR_APPROVAL_REQUIRED]       [AUTONOMOUSLY_APPROVED]
          • Dispatch MS Teams Adaptive Card  • Immediate Autonomous ERP Write-Back
          • 2-Hour Response SLA
```
...
```text
External Feeds     Part 1: Ingestion      Part 2: ML & RAG      Part 3: Orchestrator    Part 3: Specialists   Part 3: Execution
      │                   │                      │                       │                      │                     │
      │── 41.5°C Heat ───►│                      │                       │                      │                     │
      │── Highway Strike ─►│                     │                       │                      │                     │
      │                   │── Generate DOCX ────►│                       │                      │                     │
      ...
      │                   │                      │                       │                      │   (2h Approval SLA) │
      │                   │                      │                       │◄─ Complete Consensus ┘                     │
      │                   │                      │                       │── Save Audit Report (daily_agent_report)   │
```
