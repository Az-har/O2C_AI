"""
Builds O2C_AI_Databricks_Master.py and O2C_AI_Databricks_Master.ipynb
Self-contained, single-sheet executable Databricks notebooks.
"""
...
cells_data = [
    {
        "type": "markdown",
        "source": [
            "# 🚀 O2C AI MONITOR - SINGLE SHEET MASTER NOTEBOOK\n",
            "### Autonomous Dual-Engine (Engine A ML + Engine B Hybrid RAG) & Multi-Agent Delivery Delay Copilot\n",
            "\n",
            "**Complete 5-Phase Implementation in a Single Executable Sheet:**\n",
            "- **Phase 1:** Real-Time External Stream Ingestion (Live Weather & Disruption News)\n",
            "- **Phase 2:** Engine B Hybrid RAG Knowledge Engine (Okapi BM25 + FAISS Dense Vectors via RRF)\n",
            "- **Phase 3:** Engine A Predictive ML Feature Store, Geospatial Transit Modeling & Explainable AI (XAI)\n",
            "- **Phase 4:** Multi-Agent Specialist Roles (`RouteSupervisor`, `ContractAdjudicator`, `QualityMitigation`, `LLMReasoningEngine`)\n",
            "- **Phase 5:** Celonis / SAP Action Execution Layer (Delivery Blocks `LIFSK=01`, `VDATU` Updates, AP Debit Memos, Proactive 12h Clinic Notices)"
        ]
    },
    {
        "type": "markdown",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## ⚙️ Step 2: System Configuration & Path Resolution"
        ]
    },
    {
        "type": "code",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## 📂 Step 3: SAP ERP Feature Store & Data Loader\n",
            "Ingests all 10 SAP relational tables (`VBAK`, `VBAP`, `LIKP`, `LIPS`, `VTTK`, `VTTP`, `KNA1`, `KNVV`, `LFA1`, `MARA`)."
        ]
    },
    {
        "type": "code",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## 🚀 Step 4: ML Feature Engineering & Supervised Model Training (Engine A)"
        ]
    },
    {
        "type": "code",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## 📚 Step 5: Engine B - Hybrid RAG (Okapi BM25 + Dense Vectors via RRF)"
        ]
    },
    {
        "type": "code",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## 🤖 Step 6: Multi-Agent Specialist Roles & LLM Legal Reasoning (Phase 4)"
        ]
    },
    {
        "type": "code",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## 🏆 Step 7: Execute Autonomous Daily Agent Pipeline Across Orders"
        ]
    },
    {
        "type": "code",
    ...
    },
    {
        "type": "markdown",
        "source": [
            "## 📊 Step 8: Interactive Databricks Visualizations & Executive Decision Briefs"
        ]
    },
    {
        "type": "code",
...
# 1. Write O2C_AI_Databricks_Master.ipynb
ipynb_dict = {
    "cells": [
        {
            "cell_type": c["type"],
            "metadata": {},
            "execution_count": None if c["type"] == "code" else None,
...
# Clean outputs key if None
for c in ipynb_dict["cells"]:
    if c["cell_type"] == "markdown":
        c.pop("execution_count", None)
        c.pop("outputs", None)

with open("O2C_AI_Databricks_Master.ipynb", "w", encoding="utf-8") as f:
    json.dump(ipynb_dict, f, indent=2)

print("✅ Generated: O2C_AI_Databricks_Master.ipynb")

# 2. Write O2C_AI_Databricks_Master.py
py_lines = ["# Databricks notebook source\n"]
for c in cells_data:
    if c["type"] == "markdown":
        py_lines.append("# MAGIC %md\n")
        for line in c["source"]:
            py_lines.append(f"# MAGIC {line.rstrip()}\n")
    else:
        for line in c["source"]:
            py_lines.append(line)
    py_lines.append("\n# COMMAND ----------\n\n")

with open("O2C_AI_Databricks_Master.py", "w", encoding="utf-8") as f:
    f.writelines(py_lines)

print("✅ Generated: O2C_AI_Databricks_Master.py")
